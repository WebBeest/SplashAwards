...
class TabbedWidget extends TextareaWidget {
  ...
  public function formElement(FieldItemListInterface $items, $delta, array $element, array &$form, FormStateInterface $form_state) {
    ...
    $element['container_' . $count] = [
      '#title' => t('Faq item @delta: @title', ['@delta' => $count, '@title' => $title]),
      '#type' => 'details',
    ];
    $element['container_' . $count]['title'] = [
      '#type' => 'textfield',
      '#title' => t('Title'),
    ...
    $element['container_' . $count]['text'] = $text;
    $element['container_' . $count]['text']['#title'] = t('Body');

    // Attach library which automatically change the tab title.
    $element['#attached']['library'][] = 'sa/tabbed-item';
