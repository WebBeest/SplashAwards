...
/**
 * Plugin implementation of the 'position_based_image_formatter' formatter.
 *
 * @FieldFormatter(
 *   id = "position_based_image_formatter",
 *   label = @Translation("Position based image formatter"),
 *   field_types = {
 *     "image"
 *   }
 * )
 */
class PositionBasedImageFormatter extends ImageFormatterBase implements ContainerFactoryPluginInterface {
  /**
   * The current user.
   *
  ...
  public function viewElements(FieldItemListInterface $items, $langcode) {
    ...
    $entity = $items->getEntity();
    if ($entity->hasField('field_image_position')) {
      $fval = $entity->get('field_image_position')->getValue();
      if (!empty($fval[0]['value'])) {
        $elements['#attributes']['class'][] = "position-{$fval[0]['value']}";
      }
    }
